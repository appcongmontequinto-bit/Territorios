...
const PRECACHE_ASSETS = [
  ...
  // Leaflet from CDN — cache on first use
];

// Origins that are ALWAYS network-first (live data)
const NETWORK_FIRST_ORIGINS = [
  'iycqizsxmbaboaxveaeb.supabase.co',  // Supabase API
];

// Origins to cache (CDN assets)
...
self.addEventListener('install', event => {
  ...
  event.waitUntil(
    caches.open(CACHE_STATIC).then(cache => {
      // Cache what we can, ignore failures (some may not exist yet)
...
self.addEventListener('fetch', event => {
  ...
  // SUPABASE — always network, fall back to cache if offline
    ...
    return new Response(`
      ...
      <style>
        body{font-family:-apple-system,sans-serif;background:#000;color:#fff;
             ...
             min-height:100vh;margin:0;gap:16px;padding:24px;text-align:center}
        .icon{font-size:56px}
        h1{font-size:22px;font-weight:700;margin:0}
        p{font-size:15px;color:rgba(255,255,255,.6);margin:0;max-width:300px;line-height:1.6}
        button{padding:12px 24px;background:#0a84ff;color:#fff;border:none;
      ...
      <body>
        <div class="icon">📡</div>
        <h1>Sin conexión</h1>
        <p>La app está disponible pero necesitas conexión para cargar los datos de los territorios.</p>
...
self.addEventListener('notificationclick', event => {
  ...
  event.waitUntil(
    clients.openWindow(event.notification.data?.url || '/territorios.html')
  );
});
